...
let rec exec stmt (locEnv: locEnv) (gloEnv: gloEnv) (store: store) : store =
    ...
    | For(e1,e2,e3,body) -> 
          ...
          loop store0
    | Forin(item,e1,e2,body) ->
            let (loc, store1) = access item locEnv gloEnv store
            let rec loop i stores =
                if i<>(e2+1) then loop (i+1) (exec body locEnv gloEnv (setSto stores loc i))
                             else stores
            loop e1 store1
    | Switch(ex,body) ->
            let (loc1,store1) = eval ex locEnv gloEnv store
            let rec loop body stores = 
                match body with   
                      | Default(body1) -> exec body1 locEnv gloEnv stores           
                      | Case(ex1,body1,body2) ->
                        let (loc2,store2) = eval ex1 locEnv gloEnv stores
                        if loc1<>loc2 then loop body2 store2
                        else exec body1 locEnv gloEnv store2                  
                      | _ -> stores 
            loop body store1
    | Expr e ->
...
and eval e locEnv gloEnv store : int * store =
    ...
    | Prim2 (ope, e1, e2) ->
        ...
        (res, store2)
    | Prim3 (ex,e1,e2) ->
        let (i1,store1) = eval ex locEnv gloEnv store
        if i1 <> 0 then (e1,store1)
                  else  (e2,store1)
    | Inc (ope,acc,ex) ->
       let (loc, store1) = access acc locEnv gloEnv store
       let (ex1,store2) = eval ex locEnv gloEnv store1
       let r1 = getSto store1 loc 
       let res = 
           match ope with
           | "+=" -> r1 + ex1
           | "-=" -> r1 - ex1
